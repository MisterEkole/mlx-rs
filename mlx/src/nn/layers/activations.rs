...
impl Softmax {
    pub fn new(axis: i32) -> Self { Softmax { axis } }
}
...
pub fn sigmoid(x: &Array) -> Result<Array> {
    unsafe {
        let mut res_handle = crate::sys::mlx_array { ctx: std::ptr::null_mut() };
        let status = crate::sys::mlx_sigmoid(&mut res_handle, x.handle, Array::default_stream());
        x.check_status(status, res_handle)
    }
}
...
pub fn tanh(x: &Array) -> Result<Array> {
    unsafe {
        let mut res_handle = crate::sys::mlx_array { ctx: std::ptr::null_mut() };
        let status = crate::sys::mlx_tanh(&mut res_handle, x.handle, Array::default_stream());
        x.check_status(status, res_handle)
    }
}
